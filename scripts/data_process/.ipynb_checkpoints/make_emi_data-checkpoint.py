def emirnn_preprocess(bagsize,no_of_features,extractedDir,numClass,subinstanceLen, subinstanceStride,data_csv,raw_create):
    ...
    extractedDir += '/'
    if raw_create==0:
        if not (os.path.isdir(extractedDir+'RAW')):
    ...
    #         np.save(outDir + '/y_test.npy', y)
            ...
            x, y = bagData(data['x_train'], data['y_train'], subinstanceLen, subinstanceStride,numClass,numSteps,numFeats)
    #         np.save(outDir + '/x_train.npy', x)
    #         np.save(outDir + '/y_train.npy', y)
            print('Num train %d' % len(x))
            x, y = bagData(data['x_test'], data['y_test'], subinstanceLen, subinstanceStride,numClass,numSteps,numFeats)
    #         np.save(outDir + '/x_test.npy', x)
    #         np.save(outDir + '/y_test.npy', y)
            print('Num test %d' % len(x))
            x, y = bagData(data['x_val'], data['y_val'], subinstanceLen, subinstanceStride,numClass,numSteps,numFeats)
            print('Num val %d' % len(x))
