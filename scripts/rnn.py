...
class EMI_BasicLSTM(EMI_RNN):
    ...
    def _createBaseGraph(self, X):
        ...
        with tf.name_scope(scope):
            ...
            print ("Dimensions of elements of x : ",x[0].get_shape())
            # Get the LSTM output
            
            '''
            #Conv Layer 1
            # [a,b,c] -> a : no. of filters; b : input number of channels; c : output no. of channels
            filter1_ = tf.get_variable('weights1', [27,9,9], initializer=tf.truncated_normal_initializer(stddev=5e-2, dtype=tf.float32), dtype=tf.float32)
            cnn_cell = tf.nn.conv1d(x, filter1_ ,1,"SAME")
            cnn_cell = tf.transpose(cnn_cell, [1,0,2])
            print ("Shape of cnn_output :  ",cnn_cell.get_shape())
            cnn_cell = tf.keras.activations.tanh(cnn_cell)
            cnn_out = tf.unstack(cnn_cell, num=self.numTimeSteps, axis=1)
            
            
            #Conv Layer 2
            filter2_ = tf.get_variable('weights2', [9,9,3], initializer=tf.truncated_normal_initializer(stddev=5e-2, dtype=tf.float32), dtype=tf.float32)
            cnn_cell = tf.nn.conv1d(cnn_out, filter2_ ,1,"SAME")
            cnn_cell = tf.transpose(cnn_cell, [1,0,2])
            print ("Shape of cnn_output :  ",cnn_cell.get_shape())
            cnn_cell = tf.keras.activations.tanh(cnn_cell)
            cnn_out = tf.unstack(cnn_cell, num=self.numTimeSteps, axis=1)
            ...
            #Dense Layer 1
            dense_out = tf.layers.dense(cnn_cell, 32)
            #dense_out = tf.layers.dense(dense_out, 64)
            
            #Unstack the dense_out so that a sequence can be given as input to the LSTM cell.
            x = tf.unstack(dense_out, num=self.numTimeSteps, axis=1)
            '''
            ...
            '''
            When adding any layer/cell before the RNN layer, ensure that the output of that layer is given as input to the 
            RNN layer instead of 'x'.
            '''
...
class EMI_FastRNN(EMI_RNN):
    ...
    def _restoreBaseGraph(self, graph):
        ...
        if self.useDropout:
            self.keep_prob = 0.9
#             graph.get_tensor_by_name(scope + 'keep-prob:0')
        self.output = graph.get_tensor_by_name(scope + 'bag-output:0')
